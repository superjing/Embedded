...
void checkLedStatus(bool isInNetwork);
...
uint16 CurrentDetectionT1_ProcessEvent(uint8 task_id, uint16 events)
{
   ...
   //static uint8 testBuf[SN_LEN + NV_LEN] = {0xAA, 0xBB, 0xCC, 0xDD};
   ...
   if (events & DTCT_HEARTBEAT_MSG_EVT)
   {
      ...
      if (++timerCount == (DTCT_TIMER_MSG_TIMEOUT/1000 * 2))
      {
         ...
         buf[12] = HalAdcRead(HAL_ADC_CHN_AIN6, HAL_ADC_RESOLUTION_12) & 0x00FF;
         buf[13] = HalAdcRead(HAL_ADC_CHN_AIN7, HAL_ADC_RESOLUTION_12) & 0x00FF;
...
void checkLedStatus(bool isInNetwork)
{
  ...
  //P1_2: Red Led
  //P1_1: Green Led, Green Led blink when device is not in network
