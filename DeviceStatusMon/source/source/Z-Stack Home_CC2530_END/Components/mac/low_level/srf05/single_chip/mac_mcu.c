...
MAC_INTERNAL_API uint32 macMcuOverflowGetCompare(void)
{
  ...
  /* for efficiency, the 32-bit value is decoded using endian abstracted indexing */
  ((uint8 *)&compare)[UINT32_NDX0] = (uint8)T2MOVF0 & 0xFF;
  ((uint8 *)&compare)[UINT32_NDX1] = (uint8)T2MOVF1 & 0xFF;
  ((uint8 *)&compare)[UINT32_NDX2] = (uint8)T2MOVF2 & 0xFF;
...
uint32 macMcuPrecisionCount(void)
{
   ...
   */
  T2M0;
  ((uint8 *)&overflowCount)[UINT32_NDX0] = (uint8)T2MOVF0;
  ((uint8 *)&overflowCount)[UINT32_NDX1] = (uint8)T2MOVF1;
  ((uint8 *)&overflowCount)[UINT32_NDX2] = (uint8)T2MOVF2;
  ((uint8 *)&overflowCount)[UINT32_NDX3] = 0;
...
int8 macMcuGetLatestRSSI( void )
{

  /* read latest RSSI value */
  return  (int8)(RSSI & 0xFF);
}
