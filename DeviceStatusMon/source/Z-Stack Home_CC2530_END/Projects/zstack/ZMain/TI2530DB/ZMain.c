...
#ifdef DEBUG_TRACE
static void show_ieee_addr(void);
#endif
/*********************************************************************
 * @fn      main
...
#ifdef DEBUG_TRACE
void show_ieee_addr(void)
{
  uint8 i;
  uint8 *xad;
  uint8 lcd_buf[Z_EXTADDR_LEN*2+2];
  ...
  lcd_buf[i] = '\n';
  lcd_buf[Z_EXTADDR_LEN*2 + 1] = '\0';
  HalUARTWrite(0, "IEEE: ", 6);
  HalUARTWrite(0, lcd_buf, (Z_EXTADDR_LEN*2+2));
}
#endif
