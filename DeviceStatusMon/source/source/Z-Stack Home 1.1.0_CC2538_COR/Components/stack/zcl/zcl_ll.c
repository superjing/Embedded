...
ZStatus_t zclLL_RegisterCmdCallbacks( uint8 endpoint, zclLL_AppCallbacks_t *callbacks )
{
  ...
  pNewItem = osal_mem_alloc( sizeof( zclLLCBRec_t ) );
  if ( pNewItem == NULL )
    return (ZMemError);
                              ...
                              zclLLScanReq_t *pReq, uint8 seqNum )
{
  ...
  VOID osal_buffer_uint32( buf, pReq->transID );
                                  ...
                                  zclLLNwkStartReq_t *pRsp, uint8 seqNum )
{
  ...
  buf = osal_mem_alloc( ZLL_CMDLEN_NWK_START_REQ );
  if ( buf != NULL )
  {
    pBuf = buf;

    pBuf = osal_buffer_uint32( pBuf, pRsp->transID );
    pBuf = osal_cpyExtAddr( pBuf, pRsp->nwkParams.extendedPANID );

    *pBuf++ = pRsp->nwkParams.keyIndex;

    pBuf = osal_memcpy( pBuf, pRsp->nwkParams.nwkKey, SEC_KEY_LEN );
    ...
    *pBuf++ = HI_UINT16( pRsp->nwkParams.freeGrpIDEnd );
    ...
    pBuf = osal_cpyExtAddr( pBuf, pRsp->initiatorIeeeAddr );
                                 ...
                                 zclLLNwkJoinReq_t *pRsp, uint8 cmd, uint8 seqNum )
{
  ...
  buf = osal_mem_alloc( ZLL_CMDLEN_NWK_JOIN_REQ );
  if ( buf != NULL )
  {
    pBuf = buf;
                                   ...
                                   zclLLNwkUpdateReq_t *pReq, uint8 seqNum )
{
  ...
  buf = osal_mem_alloc( ZLL_CMDLEN_NWK_UPDATE_REQ );
  if ( buf != NULL )
  {
    pBuf = buf;

    pBuf = osal_buffer_uint32( pBuf, pReq->transID );
    pBuf = osal_cpyExtAddr( pBuf, pReq->extendedPANID );
                              ...
                              zclLLScanRsp_t *pRsp, uint8 seqNum )
{
  ...
  if ( buf != NULL )
  {
    ...
    *pBuf++ = HI_UINT16( pRsp->keyBitmask );
    ...
    pBuf = osal_buffer_uint32( pBuf, pRsp->responseID );
    pBuf = osal_cpyExtAddr( pBuf, pRsp->extendedPANID );
  ...
  if ( buf != NULL )
  {
    ...
    *pBuf++ = pRsp->cnt;
    ...
    for ( uint8 i = 0; i < pRsp->cnt; i++ )
    {
      ...
      pBuf = osal_cpyExtAddr( pBuf, pRec->ieeeAddr );
                                  ...
                                  zclLLNwkStartRsp_t *pRsp, uint8 seqNum )
{
  ...
  buf = osal_mem_alloc( ZLL_CMDLEN_NWK_START_RSP );
  if ( buf != NULL )
  {
    uint8 *pBuf = buf;
                                 ...
                                 zclLLNwkJoinRsp_t *pRsp, uint8 cmd, uint8 seqNum )
{
  ...
  VOID osal_buffer_uint32( buf, pRsp->transID );
                                   ...
                                   uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  buf = osal_mem_alloc( ZLL_CMDLEN_EP_INFO );
  if ( buf != NULL )
  {
    uint8 *pBuf = buf;

    pBuf = osal_cpyExtAddr( pBuf, pCmd->ieeeAddr );
                                                  ...
                                                  zclLL_AppCallbacks_t *pCBs )
{
  ...
  if ( pCBs->pfnEndpointInfo )
  {
    ...
    pCmd = (zclLLEndpointInfo_t *)osal_mem_alloc( sizeof( zclLLEndpointInfo_t ) );
    if ( pCmd )
    {
      uint8 *pBuf = pInMsg->pData;

      osal_cpyExtAddr( pCmd->ieeeAddr, pBuf );
      pBuf += Z_EXTADDR_LEN;
                                                  ...
                                                  zclLL_AppCallbacks_t *pCBs )
{
  ...
  if ( pCBs->pfnGetGrpIDsRsp )
  {
    ...
    pRsp = (zclLLGetGrpIDsRsp_t *)osal_mem_alloc( rspLen );
    if ( pRsp )
    {
      uint8 *pBuf = pInMsg->pData;
                                                  ...
                                                  zclLL_AppCallbacks_t *pCBs )
{
  ...
  if ( pCBs->pfnGetEPListRsp )
  {
    ...
    pRsp = (zclLLGetEPListRsp_t *)osal_mem_alloc( rspLen );
    if ( pRsp )
    {
      uint8 *pBuf = pInMsg->pData;
...
static ZStatus_t zclLL_ProcessInCmd_ScanReq( zclIncoming_t *pInMsg )
{
  if ( pInterPANCBs->pfnScanReq )
  {
    ...
    req.transID = osal_build_uint32( pInMsg->pData, 4 );
...
static ZStatus_t zclLL_ProcessInCmd_NwkStartReq( zclIncoming_t *pInMsg )
{
  ...
  if ( pInterPANCBs->pfnNwkStartReq )
  {
    ...
    if ( pReq )
    {
      ...
      pReq->transID = osal_build_uint32( pBuf, 4 );
      pBuf += 4;

      osal_cpyExtAddr( pReq->nwkParams.extendedPANID, pBuf );
      pBuf += Z_EXTADDR_LEN;

      pReq->nwkParams.keyIndex = *pBuf++;

      osal_memcpy( pReq->nwkParams.nwkKey, pBuf, SEC_KEY_LEN );
      pBuf += SEC_KEY_LEN;
      ...
      osal_cpyExtAddr( pReq->initiatorIeeeAddr, pBuf );
      pBuf += Z_EXTADDR_LEN;
...
static ZStatus_t zclLL_ProcessInCmd_NwkUpdateReq( zclIncoming_t *pInMsg )
{
  ...
  if ( pInterPANCBs->pfnNwkUpdateReq )
  {
    ...
    if ( pReq )
    {
      ...
      osal_cpyExtAddr( pReq->extendedPANID, pBuf );
      pBuf += Z_EXTADDR_LEN;
...
static ZStatus_t zclLL_ProcessInCmd_ScanRsp( zclIncoming_t *pInMsg )
{
  ...
  if ( pInterPANCBs->pfnScanRsp )
  {
    ...
    if ( pRsp )
    {
      ...
      pRsp->transID = osal_build_uint32( pBuf, 4 );
      pBuf += 4;
      ...
      pRsp->responseID = osal_build_uint32( pBuf, 4 );
      pBuf += 4;

      osal_cpyExtAddr( pRsp->extendedPANID, pBuf );
      pBuf += Z_EXTADDR_LEN;
  ...
  if ( pInterPANCBs->pfnDeviceInfoRsp )
  {
    ...
    if ( pRsp )
    {
      ...
      for ( uint8 i = 0; i < cnt; i++ )
      {
        ...
        osal_cpyExtAddr( pRec->ieeeAddr, pBuf );
        pBuf += Z_EXTADDR_LEN;
...
static ZStatus_t zclLL_ProcessInCmd_NwkJoinRtrRsp( zclIncoming_t *pInMsg )
{
  if ( pInterPANCBs->pfnNwkJoinRtrRsp )
  {
    ...
    rsp.transID = osal_build_uint32( pInMsg->pData, 4 );
