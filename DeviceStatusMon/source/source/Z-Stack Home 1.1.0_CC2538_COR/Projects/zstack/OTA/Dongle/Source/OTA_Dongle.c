/**************************************************************************************************
  ...
  Revised:        $Date: 2011-11-21 13:43:07 -0800 (Mon, 21 Nov 2011) $
  Revision:       $Revision: 28436 $
...
static void OTA_Dongle_IdentifyCB( zclIdentify_t *pCmd );
static void OTA_Dongle_IdentifyQueryRspCB( zclIdentifyQueryRsp_t *pRsp );
...
static zclGeneral_AppCallbacks_t OTA_Dongle_CmdCallbacks =
{
  ...
  OTA_Dongle_IdentifyCB,                  // Identify command
  NULL,                                   // Identify Trigger Effect command
  OTA_Dongle_IdentifyQueryRspCB,          // Identify Query Response command
...
uint16 OTA_Dongle_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & OTA_DONGLE_IDENTIFY_TIMEOUT_EVT )
  {
    if ( OTA_Dongle_IdentifyTime > 0 )
    {
      OTA_Dongle_IdentifyTime--;
    }

    OTA_Dongle_ProcessIdentifyTimeChange();

    return ( events ^ OTA_DONGLE_IDENTIFY_TIMEOUT_EVT );
  }
...
void OTA_Send_DongleInd()
{
  ...
  *pBuf = OTA_Dongle_devState;
  ...
  // Send the indication
  MT_BuildAndSendZToolResponse(MT_RPC_SYS_APP, MT_APP_MSG, pBuf - buffer, buffer);
}
...
static void OTA_Dongle_IdentifyCB( zclIdentify_t *pCmd )
{
  OTA_Dongle_IdentifyTime = pCmd->identifyTime;
  OTA_Dongle_ProcessIdentifyTimeChange();
}

/*********************************************************************
 * @fn      OTA_Dongle_IdentifyQueryRspCB
 *
 * @brief   Callback from the ZCL General Cluster Library when
