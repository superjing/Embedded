...
// 计算长度，将结果存放在静态变量 info_data_len 中，见变量声明部分
...
// 填充字符串 progmem_s 到数据段，起始位置为 pos，返回新的起始位置
// progmem_s 存放在程序段，使用 PSTR() 进行类型转换
// 若 pos = 0，则从头开始填充
...
// 填充字符串 s 到数据段，起始位置为 pos，返回新的起始位置
// 若 pos = 0，则从头开始填充
...
void make_tcp_ack_from_any(uint8_t *buf)
{
        ...
        make_ip(buf);
        // 计算校验和, len=8 (start from ip.src) + TCP_HEADER_LEN_PLAIN + data len
...
// 创建 ACK 数据包
// 在某些情况下，调用该函数之前必须调用 init_len_info 
// dlen 的值为要发送的 TCP(HTTP) 数据的长度
