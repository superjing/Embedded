/***************************************************************************************************
  Filename:       MT_UART.c
  Revised:        $Date: 2013-06-12 17:30:06 -0700 (Wed, 12 Jun 2013) $
  Revision:       $Revision: 34553 $
...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  if (1 == received)
  {
    ...
    //Fill up Message properties.
    pMsg->hdr.event = CMD_SERIAL_MSG;
    pMsg->msg = (uint8*)(pMsg+1);
    pMsg->msg[0] = len;
    osal_memcpy(((pMsg->msg) + 1), buffer, len);
