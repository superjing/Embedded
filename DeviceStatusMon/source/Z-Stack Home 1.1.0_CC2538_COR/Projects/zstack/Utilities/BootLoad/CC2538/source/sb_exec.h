/**************************************************************************************************
  Filename:       sb_exec.h
  Revised:        $Date: 2013-02-08 16:43:43 -0800 (Fri, 08 Feb 2013) $
  Revision:       $Revision: 33034 $
 ...
 * ------------------------------------------------------------------------------------------------
 */
#define SB_RW_BUF_LEN               SB_DEVICE_PAGE_SIZE // Use the biggest buffers possible
#define SB_BUF_HEADER_SIZE          50 // Current size is about 16, but set larger here just to be safe if it ever changed
...
// Configuration bit definitions for the sbl_hdr_t.cfg member.
...
// This is the reset vector maximum size according to the CC2538 datasheet
...
// This structure must be located at the start of the image.
typedef struct 
{
	uint32 checksum;             // ielftool-generated CRC-16 to be verified by boot loader.
	uint32 compatibility_flags;  // Currently unused. for compatibility with future bootloaders. 
	                             // enable extending the sbl_hdr_t structure in the future.
	uint32 img_status;           // Equals to SB_IMG_WRITTEN if there is an image programmed into flash 
                                     // memory, and it had been verified to have the correct checksum
	uint32 checksum_begin;       // Configuration bits. 
	uint32 checksum_end;         // Configuration bits. 
	uint32 vector_table_address;
} sbl_hdr_t;
 ...
 * None.
 *
 * @return      true or false for image(s) valid.
 */
