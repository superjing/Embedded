/******************************************************************************
  Filename:       ZDSecMgr.c
  Revised:        $Date: 2013-05-07 14:18:34 -0700 (Tue, 07 May 2013) $
  Revision:       $Revision: 34175 $
...
ZStatus_t ZDSecMgrDeviceNew( ZDSecMgrDevice_t* joiner )
{
  ...
  if ( ( ( ZG_BUILD_COORDINATOR_TYPE ) && ( ZG_DEVICE_COORDINATOR_TYPE ) )
      || ( NLME_GetShortAddr() == zgTrustCenterAddr ) )
  {
...
void ZDSecMgrInit(uint8 state)
{
  ...
  {
    if ( ( ( ZG_BUILD_COORDINATOR_TYPE ) && ( ZG_DEVICE_COORDINATOR_TYPE ) )
         || ( zgTrustCenterAddr == NLME_GetShortAddr() ) )
    {
...
void ZDSecMgrTransportKeyInd( ZDO_TransportKeyInd_t* ind )
{
  ...
  uint8 invalidExtAddr[Z_EXTADDR_LEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

  // check for distributed security
  if ( osal_ExtAddrEqual( invalidExtAddr, ind->srcExtAddr ) )
  {
    if ( ZSTACK_ROUTER_BUILD )
    {
      osal_cpyExtAddr( ind->srcExtAddr, NLME_GetExtAddr() );
      ZDSecMgrUpdateTCAddress( NLME_GetShortAddr() );
      ZDSecMgrPermitJoiningEnabled = TRUE;
    }
    else
    {
      osal_cpyExtAddr( ind->srcExtAddr, _NIB.nwkCoordExtAddress );
      ZDSecMgrUpdateTCAddress( ind->srcAddr );
    }
  }
  // load Trust Center data if needed
...
uint8 ZDSecMgrInitNV(void)
{
  ...
  rtrn |= osal_nv_item_init( ZCD_NV_TRUSTCENTER_ADDR, sizeof( zgTrustCenterAddr ),
                          &zgTrustCenterAddr );
...
static void ZDSecMgrRestoreFromNV( void )
{
  ...
  osal_nv_read( ZCD_NV_TRUSTCENTER_ADDR, 0, sizeof( zgTrustCenterAddr ), &zgTrustCenterAddr );
}
#endif // NV_RESTORE
...
{
  uint8 i, selectedId = ZDSECMGR_TC_DEVICE_MAX;
  ZStatus_t status = ZSecNoKey;
  APSME_TCLinkKey_t tcLinkKey;
  uint32 *tclkRxFrmCntr;
  uint16 originatorAddr = APSME_TRUSTCENTER_NWKADDR;
  ...
  else if ( osal_ExtAddrEqual( si->extAddr, _NIB.nwkCoordExtAddress ) )
  {
    originatorAddr = _NIB.nwkCoordAddress;
    // in case originated not from our parent, use the default TC address
  }
  ...
  for ( i = 0; i < ZDSECMGR_TC_DEVICE_MAX; i++ )
  {
    ...
    // prefer exact match over default
    ...
    else if ( APSME_IsDefaultTCLK(tcLinkKey.extAddr) )
    {
      if ( ( originatorAddr != APSME_TRUSTCENTER_NWKADDR ) && ( si->keyID == SEC_KEYID_TRANSPORT )
           && (i == 0) && ( zgUseDefaultTCLK == TRUE ) )
      {
        //if a Key is received directly from an unusual TC,
        // don't use the first element of the table initialized with the default TCLK
        continue;
      }
      if ( selectedId == ZDSECMGR_TC_DEVICE_MAX )
      {
        selectedId = i;
      }
    }
  ...
  if ( selectedId < ZDSECMGR_TC_DEVICE_MAX )
  {
    ...
    if ( si->frmCntr >= *tclkRxFrmCntr )
    {
      ...
      si->keyNvId = (ZCD_NV_TCLK_TABLE_START + selectedId);
...
ZStatus_t APSME_TCLinkKeyLoad( uint16 dstAddr, SSP_Info_t* si )
{
  ...
  for( i = 0; i < ZDSECMGR_TC_DEVICE_MAX; i++ )
  {
    ...
    {
      if ( !extAddrFound && ( zgTrustCenterAddr == APSME_TRUSTCENTER_NWKADDR ) )
      {
        status = ZSuccess;
      ...
      // Remember the last default TCLK index
      defaultTCLKIdx = i;
    }
  }
  ...
  {
    // Exact match was not found; use the last default TC Link Key
    i = defaultTCLKIdx;
    status = ZSuccess;
  }
...
/******************************************************************************
 * @fn          ZDSecMgrSaveTCAddress
 *
 * @brief       Update TC Short address and save to NV.
 *
 * @param       shortAddr - [in] short address
 *
 * @return      none
 */
void ZDSecMgrUpdateTCAddress( uint16 shortAddr )
{
  APSME_TCAddrSetup( shortAddr );
#if defined ( NV_RESTORE )
  osal_nv_write( ZCD_NV_TRUSTCENTER_ADDR, 0, sizeof( zgTrustCenterAddr ), &zgTrustCenterAddr );
#endif
}
